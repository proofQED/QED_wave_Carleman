...
1. **Run the verification engine** on the candidate file:
   ```bash
   conda activate wave && cd /local/home/cyanz/wave_PINN && python3 verify_engine.py /local/home/cyanz/wave_PINN/candidates/round_7/candidate.py
   ```

2. **Also run the JSON output** for machine-readable results:
   ```bash
   conda activate wave && cd /local/home/cyanz/wave_PINN && python3 verify_engine.py /local/home/cyanz/wave_PINN/candidates/round_7/candidate.py --json
   ```

3. **Read the candidate file** at `/local/home/cyanz/wave_PINN/candidates/round_7/candidate.py` to understand what ψ and parameters were proposed.

4. **Read the candidate's reasoning log** at `/local/home/cyanz/wave_PINN/candidates/round_7/candidate_log.md` (if it exists) to understand the search agent's intent.

5. **Write a structured verification report** to `/local/home/cyanz/wave_PINN/candidates/round_7/verification_report.md` with this exact format:
