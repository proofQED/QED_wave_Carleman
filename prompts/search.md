...
1. **Read the problem file** at `{{ problem_tex }}` to get the full operator definitions.
2. **Read the failed approaches file** at `{{ failed_approaches }}` to understand what has been tried before and WHY it failed. Do NOT repeat a failed approach unless you have a specific reason to believe a different parameter choice fixes it. Pay attention to how many times a given ansatz family has been tried — if it has failed 2+ times, move on to a different family.
{% if prev_report %}
3. **Read the previous round's verification report** at `{{ prev_report }}` to understand which conditions failed and the diagnostic expressions.
{% else %}
3. This is round 1 — there is no previous verification report.
{% endif %}
...
5. **Write the candidate file** to `{{ candidate_file }}`. The file must define:
...
6. **Write a reasoning log** to `{{ candidate_log }}` explaining:
