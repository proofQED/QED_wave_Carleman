...
1. **Run the verification engine** on the candidate file:
   ```bash
   conda activate wave && cd {{ project_root }} && python3 verify_engine.py {{ candidate_file }}
   ```

2. **Also run the JSON output** for machine-readable results:
   ```bash
   conda activate wave && cd {{ project_root }} && python3 verify_engine.py {{ candidate_file }} --json
   ```

3. **Read the candidate file** at `{{ candidate_file }}` to understand what ψ and parameters were proposed.

4. **Read the candidate's reasoning log** at `{{ candidate_log }}` (if it exists) to understand the search agent's intent.

5. **Write a structured verification report** to `{{ verification_report }}` with this exact format:

```markdown
# Verification Report — Round {{ round_num }}
